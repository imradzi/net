...
void MD5::update(std::istream &stream) {
    ...
    while (stream.good()) {
        stream.read(buff, 1024);  // note that return value of read is unusable.
        len = stream.gcount();
        update((unsigned char *)buff, len);
    }
}
...
unsigned char *MD5::raw_digest() {
    ...
    if (!s) throw std::runtime_error("cannot allocate memory");

    memset(s, 0, sizeof(unsigned char) * 16);
    if (!finalized) {
...
void MD5::transform(uint1 block[64]) {
    ...
    if (finalized) throw std::runtime_error("md5 error. ???");  // not just a user error, since the method is private
